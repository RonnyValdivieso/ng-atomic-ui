...
import { Role } from '@interfaces/aaa';
...
export class RoleFormComponent {
  ...
  @Input() set current(value: Role | null) {
    if (value) {
  ...
  protected get modalTitle(): string {
    return this.isEditing ? 'Editar rol' : 'Nuevo rol';
  }
  ...
  protected onSave(): void {
    if (this.form.valid) {
      this.save.emit(this.form.getRawValue() as RoleFormValue);
    } else {
